...
# -*- coding: utf-8 -*-
"""
Created on Sun Dec  4 00:42:50 2022
...
materialList = ['steel','magnesium','carbon']
batteryList = ['LiFePO4', 'NiMH','PbAcid-1','PbAcid-2','PbAcid-3']
batteryCellList = np.arange(8,13,1)
motorList = ['base','base_he','torque','torque_he','speed','speed_he']
parachuteList = np.arange(16,18.25,0.25)
wheelRadList = np.arange(0.2,0.75,0.05)
gearDList = np.arange(0.05,0.12,0.02)
chassisMassList = np.arange(250,610,10)
rocketFuelList = np.arange(150,290,5)
resultsTotal = np.array([[]])

for a in range(len(materialList)):
    edl_system = define_chassis(edl_system,materialList[a])
    for b in range(len(batteryList)):
        for c in range(len(batteryCellList)):
            edl_system = define_batt_pack(edl_system,batteryList[b],batteryCellList[c])
            for d in range(len(motorList)):
                edl_system = define_motor(edl_system,motorList[d])
                for e in range(len(parachuteList)):
                    para = parachuteList[e]
                    for f in range(len(wheelRadList)):
                        wRL = wheelRadList[f]
                        for g in range(len(gearDList)):
                            gDL = gearDList[g]
                            for h in range(len(chassisMassList)):
                                cML = chassisMassList[h]
                                for i in range(len(rocketFuelList)):
                                    rFL = rocketFuelList[i]
                                    try:
                                        tmax = 5000
                                        ...
                                        results = np.array([xbest[0],xbest[4],time_edl,edl_system['velocity'],xbest[1],xbest[3],xbest[2],time_rover,total_time,edl_system['rover']['telemetry']['average_velocity'],edl_system['rover']['telemetry']['distance_traveled'],edl_system['rover']['telemetry']['energy_per_distance'],edl_system_total_cost])
